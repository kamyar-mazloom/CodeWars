...
// card number, phone number or answer to your most secret question is
// still correct. However, since someone could look over your
...
function maskify(cc) {
  if (cc.length > 4) {
    let mask = "#";
    return `${mask.repeat(cc.length - 4)}${cc.slice(cc.length - 4)}`;
  } else return cc;
}
